...
def fill_template(template_name: str, variables: Dict[str, Any],
    ...
    print(f"[DEBUG_TEMPLATE] === FILL_TEMPLATE START ===")
    print(f"[DEBUG_TEMPLATE] Template: {template_name}")
    print(f"[DEBUG_TEMPLATE] Variables: {list(variables.keys())}")
    
    assert template_name.endswith('.j2'), template_name
    root_dir = os.path.dirname(os.path.dirname(__file__))
    template_path = os.path.join(root_dir, 'templates', template_name)
    print(f"[DEBUG_TEMPLATE] Template path: {template_path}")
    ...
    with open(template_path, 'r', encoding='utf-8') as fin:
        template = fin.read()
    print(f"[DEBUG_TEMPLATE] Template content length: {len(template)} chars")
    ...
    print(f"[DEBUG_TEMPLATE] Rendered content length: {len(content)} chars")
    
    with open(output_path, 'w', encoding='utf-8') as fout:
        fout.write(content)
    print(f"[DEBUG_TEMPLATE] Template written to {output_path}")
    print(f"[DEBUG_TEMPLATE] === FILL_TEMPLATE END ===")
